...
void updateInputs() {
  ...
  if (val != null) {
    val = trim(val);
    String[] parts = split(val, ','); // split by commas
    for (String p : parts) {
      if (p.startsWith("DIR:")) dir = p.substring(4);
      else if (p.startsWith("SW:")) swPressed = p.substring(3).equals("1");
      else if (p.startsWith("POT:")) potVal = int(p.substring(4));
    }
  }
}
...
void draw() {
  ...
  // Handle game inputs
if (gameOver) {
  if (swPressed) { // middle button resets game
    resetGame();
  }
} else {
  // Jump
  if (swPressed && onGround) {
    velocityY = -15;
    onGround = false;
    ducking = false;
    triggerJump();
...
if (!gameOver) {
  ...
  whiteFade = lerp(whiteFade, targetWhite, 0.02);
